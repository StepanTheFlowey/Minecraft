...
template <typename T> template <typename U> Vec3<T> Vec3<T>::operator+(const Vec3<U>& r) {
...
template <typename T> template <typename U> void Vec3<T>::operator=(const Vec3<U>& r) {
  x = static_cast<T>(r.x);
  y = static_cast<T>(r.y);
  z = static_cast<T>(r.z);
}
